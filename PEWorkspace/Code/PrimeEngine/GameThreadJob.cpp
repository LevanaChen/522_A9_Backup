...
int ClientGame::runGameFrame()
{
	...
	// auto-load the level
	if (!isLevelLoaded){
		// add the NavMeshManager as a component under GameObjectManager
		NavMeshManager *pNavMeshManager;
		PE::Handle h("NavMeshManager", sizeof(PE::Components::NavMeshManager));
		pNavMeshManager = new(h)PE::Components::NavMeshManager(*m_pContext, m_arena, h);
		m_pContext->getGameObjectManager()->addComponent(h);
		// For Assignment 8 : Automatedly loading level.
		m_pContext->getLuaEnvironment()->runString("LevelLoader.loadLevel('carracingdemo.x_level.levela', 'CarRacingDemo')");
    ...
    while (!gqh.getObject<Events::EventQueue>()->empty())
    {
			...
			|| Event_GATHER_DRAWCALLS_Z_ONLY::GetClassId() == pGeneralEvt->getClassId())
        {
			...
			if (!zOnly)
			{
				...
				// add the Debug Info
				CharacterControl::Components::ClientGameObjectManagerAddon *pClientGameOMA = m_pContext->getGameObjectManager()->getFirstComponent<CharacterControl::Components::ClientGameObjectManagerAddon>();
				CharacterControl::Components::TankController *pTC = pClientGameOMA->getTankController(pClientGameOMA->m_activatedCarID);
				SceneNode *pSN = pTC->getFirstComponent<SceneNode>();
				PhysicsNode *pPN = pSN->pPhysicsNode;

				{
					// Map Mesh
					DebugRenderer::Instance()->createMapMesh(pClientGameOMA->m_u, pClientGameOMA->m_v, true, false, false, false, 0,
						Vector3(0.05f, 0.45f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					//PEINFO("U: %f, V: %f", pClientGameOMA->m_u, pClientGameOMA->m_v);
				}

				// Track status
				{
					sprintf(PEString::s_buf, pTC->m_isOnTrack ? "Track: On" : "Track: Off", pClientGameOMA->m_activatedCarID);
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0, 
						Vector3(.80f, .05f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
				}
				// Check in status
				{
					sprintf(PEString::s_buf, pTC->m_checkedIn ? "CheckIn: On" : "CheckIn: Off");
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(.80f, .10f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
				}
				// Timer and record status
				{
					sprintf(PEString::s_buf, pTC->m_isCounting ? "Timer: On" : "Timer: Off");
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(.80f, .15f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					if (pTC->m_isCounting){
						sprintf(PEString::s_buf, "Current Timer: %f", pClientGameOMA->m_records[pClientGameOMA->m_activatedCarID].m_time);
					}
					else{
						sprintf(PEString::s_buf, "Current Timer: N/A");
					}
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(0.3f, .05f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					float lastLap = pClientGameOMA->m_records[pClientGameOMA->m_activatedCarID].m_lastRecord;
					if (lastLap == 0.0f){
						sprintf(PEString::s_buf, "Last Lap: N/A");
					}
					else{
						sprintf(PEString::s_buf, "Last Lap: %fs", lastLap);
					}
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(0.05f, .05f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					// Render the best lap
					sprintf(PEString::s_buf, "Best Lap:");
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(0.05f, .10f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					for (int i = 0; i < 3; i++){
						if (pClientGameOMA->m_records[pClientGameOMA->m_activatedCarID].m_passedRecord[i] == 0.0f){
							sprintf(PEString::s_buf, "---.---");
						}
						else{
							sprintf(PEString::s_buf, "%fs", pClientGameOMA->m_records[pClientGameOMA->m_activatedCarID].m_passedRecord[i]);
						}
						DebugRenderer::Instance()->createTextMesh(
							PEString::s_buf, true, false, false, false, 0,
							Vector3(0.15f, .10f + i * 0.05f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					}

				}
				{
					// Network Info
					sprintf(PEString::s_buf, "Ping: %3.0f ms", pClientGameOMA->m_networkPing * 1000);
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(0.55f, .05f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
				}
				{
					// PhysicsDebug Info
					sprintf(PEString::s_buf, "Velocity: %f", pPN->velocity);
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(0.7f, .25f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					sprintf(PEString::s_buf, "Rotation: %f", pPN->rotation);
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(0.7f, .30f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					sprintf(PEString::s_buf, "Rolling: %f", pPN->rr);
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(0.7f, .35f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					sprintf(PEString::s_buf, "Friction: %f", pPN->u);
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(0.7f, .40f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					sprintf(PEString::s_buf, "Resistence: %f", pPN->resistence);
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(0.7f, .45f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
				}
				{
					// Game Instruction
					if (!isConnected){
						sprintf(PEString::s_buf, "Press P to start the game");
						DebugRenderer::Instance()->createTextMesh(
							PEString::s_buf, true, false, false, false, 0,
							Vector3(0.35f, .45f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					}
					sprintf(PEString::s_buf, "Key Instruction W:forward, S:backward, Left Arrow: turn left, Right Arrow: turn right");
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(0.10f,0.90f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
					sprintf(PEString::s_buf, "K: reload the car position, L: stop the timer");
					DebugRenderer::Instance()->createTextMesh(
						PEString::s_buf, true, false, false, false, 0,
						Vector3(0.10f, 0.95f, 0), 1.0f, m_pContext->m_gameThreadThreadOwnershipMask);
				}
		...
		else if (Event_JOIN_GAME::GetClassId() == pGeneralEvt->getClassId())
		{
			ClientNetworkManager *cNetworkManager = m_pContext->getGameObjectManager()->getFirstComponent<ClientNetworkManager>();
			if (cNetworkManager){
				cNetworkManager->clientConnectToTCPServer("127.0.0.1", PE_SERVER_PORT);
				isConnected = true;
			}
		}
