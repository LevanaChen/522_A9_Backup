...
// For Assignment 8 : Add including PhysicsNode in GameObjectManager
	...
	if (!haveObject)
	{
		...
		else
		{
			if (pRealEvent->hasCustomOrientation)
			{
				...
				if (StringOps::endswith(pRealEvent->m_meshFilename, "NavMesh.mesha")){
					// This is the nav mesh, no need to add as the physics node
					MeshCPU mcpu(*m_pContext, m_arena);
					mcpu.ReadMesh(pRealEvent->m_meshFilename, pRealEvent->m_package, "");
					PositionBufferCPU* pbCPU = mcpu.m_hPositionBufferCPU.getObject<PositionBufferCPU>();
					Vector3 vertices[4];
					for (int i = 0; i < 4; i++){
						vertices[i] = pSN->m_base * Vector3(pbCPU->m_values[i * 3], pbCPU->m_values[i * 3 + 1], pbCPU->m_values[i * 3 + 2]);
						vertices[i].m_y = 0.0f;	// we don't need y for the nav mesh
					}
					NavMeshManager *pNavMeshMgr = getFirstComponent<NavMeshManager>();
					if (pNavMeshMgr){
						pNavMeshMgr->generatePolygon(vertices[0], vertices[2], vertices[3], vertices[1]);
					}
				}
				else if (StringOps::endswith(pRealEvent->m_meshFilename, "finishline.mesha")){
					// load the finishline mesh into the NavMeshManager
					MeshCPU mcpu(*m_pContext, m_arena);
					mcpu.ReadMesh(pRealEvent->m_meshFilename, pRealEvent->m_package, "");
					PositionBufferCPU* pbCPU = mcpu.m_hPositionBufferCPU.getObject<PositionBufferCPU>();
					Vector3 vertices[4];
					...
					NavMeshManager *pNavMeshMgr = getFirstComponent<NavMeshManager>();
					if (pNavMeshMgr){
						pNavMeshMgr->createStartPolygon(vertices[0], vertices[2], vertices[3], vertices[1]);
					}
				}
				else{
					// For Assignment 8 : Initially create physics node to each mesh and add it into scene node
					...
					//pMainPN->AABB_max = mcpu.AABB_max + pSN->m_base.getPos();	// This is wrong since I scale the grass actually
					//pMainPN->AABB_min = mcpu.AABB_min + pSN->m_base.getPos();
					pMainPN->AABB_max = pSN->m_base * mcpu.AABB_max;
					pMainPN->AABB_min = pSN->m_base * mcpu.AABB_min;

					if (StringOps::endswith(pRealEvent->m_meshFilename, "grassland.mesha"))
					{
						pMainPN->m_type = PhysicsNode::GRASS;
					}
					else if ( StringOps::endswith(pRealEvent->m_meshFilename, "RoadLeft45.mesha") || 
							  StringOps::endswith(pRealEvent->m_meshFilename, "RoadRight45.mesha") ||
							  StringOps::endswith(pRealEvent->m_meshFilename, "RoadStraight.mesha") )
					{
						pMainPN->m_type = PhysicsNode::ROAD;
					}
