...
void DefaultGameControls::handleKeyboardDebugInputEvents(Event *pEvt)
{
	...
	else if (Event_KEY_P_UP::GetClassId() == pEvt->getClassId())
	{
		Handle h("Event", sizeof(Event_JOIN_GAME));
		Event_JOIN_GAME *joinGameEvt = new(h)Event_JOIN_GAME;
		m_pQueueManager->add(h, QT_GENERAL);
	}
...

{
	...
	else if (Event_BUTTON_START_UP::GetClassId() == pEvt->getClassId()){
		Handle h("Event", sizeof(Event_JOIN_GAME));
		Event_JOIN_GAME *joinGameEvt = new(h)Event_JOIN_GAME;
		m_pQueueManager->add(h, QT_GENERAL);
	}
