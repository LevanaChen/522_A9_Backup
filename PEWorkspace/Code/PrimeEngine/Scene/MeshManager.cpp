...
PE::Handle MeshManager::getAsset(const char *asset, const char *package, int &threadOwnershipMask)
{
	...
	else if (StringOps::endswith(asset, "mesha"))
	{
		...
		if (StringOps::endswith(asset, "car.mesha"))
		{
			PositionBufferCPU *pBufferCPU = (PositionBufferCPU *)mcpu.m_hPositionBufferCPU.getObject();
			for (int i = 0; i < pBufferCPU->m_values.m_size; i++){
				pBufferCPU->m_values[i] /= 100.0f;
			}
		}
		...
		if (StringOps::endswith(asset, "NavMesh.mesha")){
			// this is the nav mesh, no AABB needed
			int test = 0;
		}
		else{
			// For Assignment 8 : Transit AABB information from MeshCPU into Mesh
			pMesh->AABB_max = mcpu.AABB_max;
			pMesh->AABB_min = mcpu.AABB_min;
		}
