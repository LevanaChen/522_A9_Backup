...
void DebugRenderer::createMapMesh(float x, float y, bool isOverlay2D, bool is3D, bool is3DFacedToCamera,
	...
	int &threadOwnershipMask)
{
	if (EnableDebugRendering && m_numAvaialble)
	{
		// int index = m_hAvailableSNs[--m_numAvaialble];
		Handle &h = m_hMapSN;
		MapSceneNode *pMapSN = 0;
		if (h.isValid())
		{
			//scene node has already been created
			pMapSN = h.getObject<MapSceneNode>();
			//assert(pMapSN->isEnabled() == false); // this SN should never be in the available list if it is enabled
			pMapSN->setSelfAndMeshAssetEnabled(true);
		}
		else
		{
			h = PE::Handle("Map_SCENE_NODE", sizeof(MapSceneNode));
			pMapSN = new(h)MapSceneNode(*m_pContext, m_arena, h);
			pMapSN->addDefaultComponents();
			addComponent(h);
		}
		m_mapLifeTime = timeToLive;
		MapSceneNode::DrawType drawType = MapSceneNode::InWorld;
		if (isOverlay2D)
		{
			drawType = MapSceneNode::Overlay2D;
		...
		if (is3DFacedToCamera)
			drawType = MapSceneNode::Overlay2D_3DPos;
		pMapSN->loadFromPos_needsRC(x, y, drawType, threadOwnershipMask);
		pMapSN->m_base.setPos(pos);
		pMapSN->m_scale = scale;
	}
}
