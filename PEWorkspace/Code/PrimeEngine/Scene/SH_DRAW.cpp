...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
	...
	// print the nav mesh box
	NavMeshManager *pNavMeshManager = m_pContext->getGameObjectManager()->getFirstComponent<NavMeshManager>();
	if (pNavMeshManager){
		for (int i = 0; i < pNavMeshManager->m_polygons.size(); i++){
			Vector3 v0 = pNavMeshManager->m_polygons[i].m_vertices[0];
			Vector3 v1 = pNavMeshManager->m_polygons[i].m_vertices[1];
			Vector3 v2 = pNavMeshManager->m_polygons[i].m_vertices[2];
			Vector3 v3 = pNavMeshManager->m_polygons[i].m_vertices[3];
			v0.m_y += 0.1f;
			v1.m_y += 0.1f;
			v2.m_y += 0.1f;
			v3.m_y += 0.1f;
			Vector3 linepts1[] = { v0, red, v1, red };
			Vector3 linepts2[] = { v1, red, v2, red };
			Vector3 linepts3[] = { v2, red, v3, red };
			Vector3 linepts4[] = { v3, red, v0, red };
			DebugRenderer::Instance()->createLineMesh(false, matrix, &linepts1[0].m_x, 2, 0);
			DebugRenderer::Instance()->createLineMesh(false, matrix, &linepts2[0].m_x, 2, 0);
			DebugRenderer::Instance()->createLineMesh(false, matrix, &linepts3[0].m_x, 2, 0);
			DebugRenderer::Instance()->createLineMesh(false, matrix, &linepts4[0].m_x, 2, 0);
		}
	}
