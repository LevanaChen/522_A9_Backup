...
void MeshCPU::ReadMesh(const char *filename, const char *package, const char *tag)
{
	...
	// For Assignment 8 : Calculate AABB information, and store in MeshCPU
	PositionBufferCPU *pos = m_hPositionBufferCPU.getObject<PositionBufferCPU>();
	int len = (pos->m_values.m_size) / 3;
	if (len == 0)
	{
		AABB_min = Vector3(0,0,0);
		AABB_max = Vector3(0,0,0);
	}
	else
	{
		AABB_min = Vector3(pos->m_values[0], pos->m_values[1], pos->m_values[2]);
		AABB_max = Vector3(pos->m_values[0], pos->m_values[1], pos->m_values[2]);
		for (int i=1; i<len; i++)
		{
			int x = 3*i;
			int y = 3*i +1;
			int z = 3*i +2;
			if (pos->m_values[x] < AABB_min.getX())
				AABB_min.m_x = pos->m_values[x];
			if (pos->m_values[y] < AABB_min.getY())
				AABB_min.m_y = pos->m_values[y];
			if (pos->m_values[z] < AABB_min.getZ())
				AABB_min.m_z = pos->m_values[z];
			if (pos->m_values[x] > AABB_max.getX())
				AABB_max.m_x = pos->m_values[x];
			if (pos->m_values[y] > AABB_max.getY())
				AABB_max.m_y = pos->m_values[y];
			if (pos->m_values[z] > AABB_max.getZ())
				AABB_max.m_z = pos->m_values[z];
		}
	}
